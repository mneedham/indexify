...
pub struct NamespaceEndpointState {
    ...
    pub metrics: metrics::server::Metrics,
}
...
impl Server {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        let namespace_endpoint_state = NamespaceEndpointState {
            ...
            metrics: crate::metrics::server::Metrics::new(),
        };
...
async fn upload_file(
    ...
    while let Some(file) = files.next_field().await.unwrap() {
        ...
        state.metrics.node_content_uploads.inc();
        state.metrics.node_content_bytes_uploaded.inc_by(size_bytes);
    }
    Ok(())
}
